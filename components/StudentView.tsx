...
import { Search, BookOpen, MessageCircle, Send } from 'lucide-react';
import { Book, Category, User, ThemeColor, LibraryMessage } from '../types';
...
export const StudentView: React.FC<StudentViewProps> = ({ books, categories, handleBorrow, currentUser, themeColor, messages, sendMessage }) => {
      ...
      <div className="relative h-48 rounded-3xl overflow-hidden shadow-lg">
       ...
       <div className="flex gap-4 border-b border-gray-200 dark:border-gray-700 pb-2">
        <button
          ...
          className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-colors ${activeTab === 'browse' ? `bg-${themeColor}-600 text-white shadow-md shadow-${themeColor}-500/30` : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
        >
          <BookOpen size={18} /> Browse Books
        </button>
        <button
          onClick={() => setActiveTab('chat')}
          className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-colors ${activeTab === 'chat' ? `bg-${themeColor}-600 text-white shadow-md shadow-${themeColor}-500/30` : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {filteredBooks.map(book => (
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl dark:shadow-black/50 transition-all border border-gray-100 dark:border-gray-700 flex flex-col h-full"
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <div className="mb-auto">
                      ...
                      <h3 className="font-bold text-gray-800 dark:text-gray-100 text-lg leading-tight mt-1 mb-1 font-serif">{book.title}</h3>
